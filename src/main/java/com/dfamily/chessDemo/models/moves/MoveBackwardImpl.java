 ...
 * @author johns
 */
public class MoveBackwardImpl implements MoveBackward {
    private static MoveBackward instance;

    private MoveBackwardImpl() {
    }

    public static MoveBackward getInstance() {
        if (instance == null) {
            instance = new MoveBackwardImpl();
        }
        return instance;
    }
        ...
        return from.stream()
                .map(c -> c.getB().getCases().stream()
                        ...
                        .filter(c2 -> {
                            ...
                            return ("Black".equals(first.getP().getOwner().getColor()) && first.getNameID().charAt(1)+distance < 8 && 
                                c2.getNameID().charAt(1) == first.getNameID().charAt(1)+distance+1) ||
                                ("White".equals(first.getP().getOwner().getColor()) && first.getNameID().charAt(1)-distance > 1 && 
                                c2.getNameID().charAt(1) == first.getNameID().charAt(1)-distance-1);
                        })
