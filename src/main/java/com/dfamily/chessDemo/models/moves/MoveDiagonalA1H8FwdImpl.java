 ...
 * @author johns
 */
public class MoveDiagonalA1H8FwdImpl implements MoveDiagonalA1H8 {
    private static MoveDiagonalA1H8 instance;

    private MoveDiagonalA1H8FwdImpl() {
    }

    public static MoveDiagonalA1H8 getInstance() {
        if (instance == null) {
            instance = new MoveDiagonalA1H8FwdImpl();
        }
        return instance;
    }
                ...
                .map(c -> c.getB().getCases().stream()
                        ...
                        .filter(c2 -> {
                            ...
                            return (distanceX == distanceY && 
                                    (("White".equals(first.getP().getOwner().getColor()) && (first.getNameID().charAt(1)+distanceY < 8 && first.getNameID().charAt(1)+distanceX < 'h') && 
                                    ((c2.getNameID().charAt(0) == first.getNameID().charAt(0)+distanceX+1)&&(c2.getNameID().charAt(1) == first.getNameID().charAt(1)+distanceY+1))) ||                                    
                                    ("Black".equals(first.getP().getOwner().getColor()) && (first.getNameID().charAt(1)-distanceY > 1 && first.getNameID().charAt(1)-distanceX < 'a') && 
                                    ((c2.getNameID().charAt(0) == first.getNameID().charAt(0)-distanceX-1)&&(c2.getNameID().charAt(1) == first.getNameID().charAt(1)-distanceY-1)))
                                    ));
                        })
