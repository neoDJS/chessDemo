 ...
 * @author johns
 */
public class MoveDiagonalA8H1FullImpl implements MoveDiagonalA8H1 {
    private static MoveDiagonalA8H1 instance;

    private MoveDiagonalA8H1FullImpl() {
    }

    public static MoveDiagonalA8H1 getInstance() {
        if (instance == null) {
            instance = new MoveDiagonalA8H1FullImpl();
        }
        return instance;
    }
                ...
                .map(c -> c.getB().getCases().stream()
                        ...
                        .filter(c2 -> {
                            ...
                            return (distanceX == distanceY && 
                                    (((first.getNameID().charAt(1)+distanceY < 8 && first.getNameID().charAt(1)-distanceX < 'a') && 
                                    ((c2.getNameID().charAt(0) == first.getNameID().charAt(0)-distanceX-1)&&(c2.getNameID().charAt(1) == first.getNameID().charAt(1)+distanceY+1))) ||
                                    ((first.getNameID().charAt(1)-distanceY > 1 && first.getNameID().charAt(1)+distanceX < 'h') && 
                                    ((c2.getNameID().charAt(0) == first.getNameID().charAt(0)+distanceX+1)&&(c2.getNameID().charAt(1) == first.getNameID().charAt(1)-distanceY-1)))
                                    ));
                        })
